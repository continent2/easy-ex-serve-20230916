...
const getrandomrows =async ( tablename , jfilter , N ) =>{
  let aresp= await db[tablename].findAll({ raw: true , order: db.Sequelize.literal('rand()'), limit: N , where : jfilter })
...
const findall_select_columns=async(table , jfilter,acols)=>{	
	if ( acols && acols.constructor === Array){
